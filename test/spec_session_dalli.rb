begin
  require 'dalli_session'
  require 'rack/mock'
  require 'thread'

  describe Rack::Session::DalliSession do
    session_key = Rack::Session::DalliSession::DEFAULT_OPTIONS[:key]
    ...
    it "faults on no connection" do
      if RUBY_VERSION < "1.9"
        lambda{
          Rack::Session::DalliSession.new(incrementor, :memcache_server => 'nosuchserver')
        }.should.raise
      else
        lambda{
          Rack::Session::DalliSession.new(incrementor, :memcache_server => 'nosuchserver')
    ...
    it "connects to existing server" do
      lambda{
        test_pool = Rack::Session::DalliSession.new(incrementor, :namespace => 'test:rack:session')
      }.should.not.raise
    end

    it "creates a new cookie" do
      pool = Rack::Session::DalliSession.new(incrementor)
    ...
    it "maintains freshness" do
      pool = Rack::Session::DalliSession.new(incrementor, :expire_after => 3)
      res = Rack::MockRequest.new(pool).get('/')
    ...
    it "updates deep hashes correctly" do
      ...
      pool = Rack::Session::DalliSession.new(hash_check)
      req = Rack::MockRequest.new(pool)

      res0 = req.get("/")
      session_id = (cookie = res0["Set-Cookie"])[session_match, 1]
      ses0 = pool.pool.get('rack:session:'+session_id, true)

      res1 = req.get("/", "HTTP_COOKIE" => cookie)
      ses1 = pool.pool.get('rack:session:'+session_id, true)
  ...
  $stderr.puts "Skipping Rack::Session::DalliSession tests. Start memcached and try again."
rescue LoadError
  $stderr.puts "Skipping Rack::Session::DalliSession tests (Dalli is required). `gem install dalli` and try again."
end
